...
class Visualizer(nn.Module):
    ...
    def forward(self, frame:Image, bboxes:torch.Tensor,sentence:str):
        '''
        ...
        for i,bbox in enumerate(bboxes):
            ...
            draw.rectangle(bbox.numpy(), outline =color, width = 3)
            draw.rectangle([bbox[0],bbox[1]-20,bbox[0]+20 + len("{}".format(i))*3,bbox[1]], fill=color)
            draw.text((bbox[0]+5, bbox[1]-20),"{}".format(i) , fill=op_color, font=font)
        font_sen = ImageFont.truetype("segoeui.ttf", 20)
